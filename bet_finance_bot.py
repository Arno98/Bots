...
class Bet_Finance_Bot():
	...
	def __init__(self):
		...
		def percent_message(message):
			bot.send_message(message.chat.id, "Ваш процент: " + str(self.percent) + "% от сумы текущего баланса, что составляет " + "{:.2f}".format(float(self.current_bet_ammount)))
		...
		def add_result(message):
			if self.current_bet != 0:
				...
				keyboard.row('+')
				keyboard.row('-')
				bot.send_message(message.chat.id, "Введите результат ставки ('+' для выигрыша, '-' для проигрыша):", reply_markup=keyboard)
		...
		def get_result(message):
			if message.text == '+':
				bot.send_message(message.chat.id, "Поздравляю!\n\nВаш баланс увеличен на " + "{:.2f}".format(float(self.current_bet_ammount * self.current_bet)), reply_markup=keyboard)
				self.balance += self.current_bet_ammount * self.current_bet
				self.current_bet_ammount = (self.balance * self.percent) / 100
				self.current_bet = 0
			elif message.text == '-':
				bot.send_message(message.chat.id, "Очень жаль!\n\nВаш баланс уменьшился на " + "{:.2f}".format(float(self.current_bet_ammount)), reply_markup=keyboard)
				self.current_bet_ammount = (self.balance * self.percent) / 100
				self.current_bet = 0																				
			else:
